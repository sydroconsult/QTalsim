...
from qgis.PyQt.QtWidgets import QAction, QTableWidgetItem, QComboBox, QFileDialog, QLabel
...
class QTalsim:
    ...
    def __init__(self, iface):
        ...
        #Field Names
        self.IDSoil = "ID_Soil" #has to be the same name as in soilParameter.csv
    ...
    def start_loading(self):
        ...
        self.loadingLabel.move(cursor_pos.x() + 20, cursor_pos.y() + 20)  #offsets can be changed here
        ...
        for feature in layer.getFeatures():
            ...
            for fid in candidate_ids:
                ...
                if feature.geometry().overlaps(other_feature.geometry()):
                    #print(f"Feature {feature.id()} overlaps with feature {other_feature.id()}")
    ...
    def editOverlappingFeatures(self, layer):
        ...
        for feature in layer.getFeatures(): #Looping over all features in input layer
            candidate_ids = index.intersects(feature.geometry().boundingBox()) #Possible overlapping candidates of feature
            for fid in candidate_ids: #loop over possibly overlapping features
                if fid == feature.id() or fid in checked_ids:
                    continue #if the feature itself the overlapping feature or the candidate was already visited --> continue to next candidate
                ...
                if feature.geometry().overlaps(other_feature.geometry()):
                    ...
                    if feature.geometry().contains(other_feature.geometry()):
                        ...
                        changes_made = True
                        if feature.geometry().overlaps(other_feature.geometry()): #if the features are still overlapping
                    ...
                    #2nd possibility: Feature is larger than other_feature  
    ...
    def clipLayer(self, layer, clipping_layer):
        ...
        errors_layer = result['ERROR_OUTPUT']
        '''
        if invalid_layer.featureCount() > 0:
            print("There are invalid geometries.")

            for feature in errors_layer.getFeatures():
                print("Feature ID {}: {}".format(feature.id(), feature['message']))
        else:
    ...
    def checkGaps(self, layer, extent):
        '''
        Checks for Gaps
        layer: layer where polygons are eliminated
        extent: layer (e.g. dissolved catchment area layer) that defines the boundary of layer
        '''
    ...
    def fillGaps(self, layer, extent, mode):
        ...
        result_merge = processing.run("native:mergevectorlayers", {'LAYERS': [layer, gapsLayer],  'OUTPUT': 'TEMPORARY_OUTPUT'})
    ...
    def selectEZG(self):
        ...
        outputLayer = processing.run("native:deleteduplicategeometries", {'INPUT': self.ezgLayer ,'OUTPUT':'TEMPORARY_OUTPUT'})['OUTPUT']
    ...
    def selectSoil(self):
        ...
        self.soilFields = self.soilLayer.fields()

        self.dlg.comboboxSoilFields.clear()
        self.dlg.comboboxSoilFields.addItems([field.name() for field in self.soilFields])
        self.fillSoilTable()
    ...
    def fillSoilTable(self):
        ...
        self.dlg.tableSoilMapping.setHorizontalHeaderLabels(['Soil Layer Fields', 'Talsim Soil Parameters'])
        ...
        for row, data in enumerate(soilTextures):
            ...
            if data != 'Name' and data != self.IDSoil:
    ...
    def checkOverlappingSoil(self):
        self.checkOverlappingFeatures(self.soilLayer) #oben gehört noch ausgebessert, dass Overlaps usw. nicht schon beim Layer bestätigen sind sondern erst danach, wenn User das möchte, aber vor Overlaps löschen noch Möglichkeit einzelne Typen zu löschen und zu eliminieren
    ...
    def deleteOverlappingSoilFeatures(self):
        ...
        while changes_made and iteration < max_iteration:
            outputLayer, changes_made = self.editOverlappingFeatures(self.soilLayer)
            iteration += 1
        ...
        #resultDeleteHoles = processing.run("native:deleteholes", {'INPUT':outputLayer,'MIN_AREA':0,'OUTPUT':'TEMPORARY_OUTPUT'})
        #outputLayer = resultDeleteHoles['OUTPUT']
        self.soilLayer = outputLayer
        self.soilLayer.setName("Boden")
        QgsProject.instance().addMapLayer(self.soilLayer)

    def showSoilTypes(self):
        self.start_loading()
        #populate soilTypesDelete
        self.dlg.soilTypesDelete.clear()
        self.soiltypeField = self.dlg.comboboxSoilFields.currentText()
        field_index = self.soilFields.indexFromName(self.soiltypeField)
    
        # Check if the field index is valid
        if field_index >= 0:
            field_values = [feature.attributes()[field_index] for feature in self.soilLayer.getFeatures()]
            self.unique_soiltypes = set(field_values) #Fill data
            self.unique_soiltype_types = set(type(value) for value in self.unique_soiltypes)
            self.dlg.soilTypesDelete.addItems([str(soiltype) for soiltype in self.unique_soiltypes]) #hier speichern von tatsächlichem Typ (zB Float) damit man beim löschen unten diesen wieder auswählen kann
        self.stop_loading()
    ...
    def deleteSoilTypes(self):
        ...
        for feature in self.soilLayer.getFeatures(): 
        ...
        for fid in ids_to_delete:
            self.soilLayer.deleteFeature(fid)
        print(f"Es wurden {len(ids_to_delete)} Features gelöscht.")
    ...
    def checkGapsSoil(self):
        self.checkGaps(self.soilLayer, self.clippingEZG)
    ...
    def fillGapsSoil(self):
        ...
        if self.dlg.comboboxModeEliminateSoil.currentText() == 'Smallest Area':
            ...
            mode = 0
        self.soilLayer = self.fillGaps(self.soilLayer,self.clippingEZG, mode)
        QgsProject.instance().addMapLayer(self.soilLayer)
    ...
    def createSoilLayer(self):
        ...
        '''
        #Create Layer
        self.soilTalsim = QgsVectorLayer(f"Polygon?crs={self.soilLayer.crs().authid()}", "Talsim Soil", "memory")
        ...
        mem_layer_data = self.soilTalsim.dataProvider()
        ...
        for row in range(self.dlg.tableSoilMapping.rowCount()): #Loop over all entries of the Soil Mpaping Table
            print(self.dlg.tableSoilMapping.cellWidget(row, 1).currentText())
            ...
            if self.soilTalsim.fields().indexOf(old_field) != -1:
                type_old = self.soilTalsim.fields().field(old_field).type()
                if type_old != type:
        ...
        self.soilTalsim.dataProvider().addAttributes(new_fields) #create new fields with the talsim parameters
        self.soilTalsim.updateFields()
        
        #Populate soil parameter fields in Talsim soil layer
        self.soilTalsim.startEditing()
        try:
            for feature in self.soilTalsim.getFeatures():
                for old_field, new_field in value_mapping.items():
                    if old_field == 'Parameter not available':
                        ...
                        new_field_type = self.soilTalsim.fields().field(new_field).type()
                ...
                try:
                    self.soilTalsim.updateFeature(feature)
                except Exception as e:
                    self.soilTalsim.updateFeature(feature)
        ...
        self.soilTalsim.dataProvider().reloadData()
        QgsProject.instance().addMapLayer(self.soilTalsim)
        #Dissolve the layer using the talsim soil parameters
        resultDissolve = processing.run("native:dissolve", {'INPUT':self.soilTalsim,'FIELD': self.soilFieldNames,'SEPARATE_DISJOINT':True,'OUTPUT':'TEMPORARY_OUTPUT'}, feedback=self.feedback)
    ...
    def selectLanduse(self):
        '''
            Clips Landuse Layer and deletes Double Geometries
        '''
        self.start_loading()
    ...
    def updateCsvDelimiter(self):
        try:
            ...
            #csvList = list(self.dfLanduseTalsim.columns.values)
            self.dlg.landuseParameter.clear()
            self.dlg.landuseParameter.addItems([header for header in self.dfLanduseTalsim.columns.values])

        except Exception as e:
            self.log_to_qtalsim_tab(f"Error: {e}", Qgis.Warning)
            print("Error:", str(e))

    def landuseParameters(self):
        self.selected_landuse_parameters = [item.text() for item in self.dlg.landuseParameter.selectedItems()] 
        if not self.fieldLanduseID in self.selected_landuse_parameters:
            self.log_to_qtalsim_tab("There is no unique identifier 'ID_LNZ' chosen", Qgis.Critical)
            self.selected_landuse_parameters = None
        else:
            
            self.log_to_qtalsim_tab(f"Parameters chosen: {self.selected_landuse_parameters}", Qgis.Info)
    
    def checkOverlappingLanduse(self):
        self.checkOverlappingFeatures(self.landuseLayer)

    def landuseAssigning(self):
        # tableLanduseMapping --> implement mapping here?
    ...
    def confirmLanduseClassification(self):
        ...
        #self.landuseTalsim.commitChanges() kommentiert am 24.11 --> nicht notwendig?!
        value_mapping = {}
        ...
        print("features:", self.landuseTalsim.featureCount())
        print("features landuse alt:", self.landuseLayer.featureCount())
        ids_to_delete = [] 
    ...
    def deleteOverlappingLanduseFeatures(self):
        ...
        iteration = 0
        while changes_made and iteration < max_iteration:
            outputLayer, changes_made = self.editOverlappingFeatures(self.landuseTalsim)
            iteration += 1
        ...
        self.landuseTalsim = outputLayer
        self.landuseTalsim.setName("Landuse")
        QgsProject.instance().addMapLayer(self.landuseTalsim)
    ...
    def fillGapsLanduse(self):
        ...
        if self.dlg.comboboxModeEliminateSoil.currentText() == 'Smallest Area':
            ...
            mode = 0
        self.landuseTalsim = self.fillGaps(self.landuseTalsim,self.clippingEZG, mode)
        QgsProject.instance().addMapLayer(self.landuseTalsim)
    ...
    def performIntersect(self):
        ...
        mem_layer_data.addAttributes([QgsField(ezgFeatureID, QVariant.Int)])
        ezgLayer1.updateFields()
        
        ezgLayer1.startEditing()
        for feature in ezgLayer1.getFeatures():
            #feature.setAttribute(feature.fieldNameIndex(ezgFeatureID), feature.id())
            feature[ezgFeatureID] = feature.id()
            ezgLayer1.updateFeature(feature)
        ezgLayer1.commitChanges()
        ...
        #intersectedLayer = self.fillGaps(intersectedLayer, self.clippingEZG, 0)
        #Edit overlapping features
        QgsProject.instance().addMapLayer(intersectedLayer)
        
        #Calculate and store area of every catchment area
        ezgAreas = {}
        for feature in ezgLayer1.getFeatures():
            ezgAreas[feature[ezgFeatureID]] = feature.geometry().area()

        #Dissolve Layer 1
        dissolve_list = []
        dissolve_list.append(ezgFeatureID)
        ...
        resultSplit = processing.run("native:splitvectorlayer", {'INPUT': intersectedDissolvedLayer,'FIELD':ezgFeatureID,'PREFIX_FIELD':True,'FILE_TYPE':0,'OUTPUT':'TEMPORARY_OUTPUT'})
        ...
        QgsProject.instance().addMapLayer(ezgLayer1) 
        ...
        for key in ezgAreas:
            if key in sum_areas:
                if round(ezgAreas[key],0) != round(sum_areas[key],0):
                    self.log_to_qtalsim_tab(f'Catchment area with Feature-ID {key} has a different area {ezgAreas[key]} than the sum of all features in this catchment area {sum_areas[key]}.', Qgis.Warning)
        
        '''
            Create .EFL
        '''
        eflFieldList = []
        eflFieldList.append(ezgFeatureID) #ID of catchment area
        ...
        self.eflLayer = QgsVectorLayer('None', 'eflLayer', 'memory') #create layer

        #Add Fields
        eflLayerDP = self.eflLayer.dataProvider()
        eflFields = []
        try:
            for fieldname in eflFieldList:
                if fieldname == self.fieldNameAreaEFL:
                    eflFields.append(QgsField(self.fieldNameAreaEFL, QVariant.Double))
                else:
                    field = self.finalLayer.fields().field(fieldname)
                    eflFields.append(field)
        except Exception as e:
            self.log_to_qtalsim_tab(e, level=Qgis.Erro)
        ...
        self.eflLayer.startEditing()
        for feature in self.finalLayer.getFeatures():
            newFeature = QgsFeature(self.eflLayer.fields())
            area = feature.geometry().area()
            ezg = feature[ezgFeatureID]
            ezgArea = ezgAreas[ezg]
            percentage = (area/ezgArea)*100
            for field in eflFieldList:
                if field == self.fieldNameAreaEFL:
                    newFeature.setAttribute(field, percentage)
                else:
                    newFeature.setAttribute(field, feature[field])
            self.eflLayer.addFeature(newFeature)
        self.eflLayer.commitChanges()
        ...
        for feature in self.eflLayer.getFeatures():
            subbasin_id = feature['SubBasinId']  # Replace 'SubBasinId' with your field name
            percentage_share = feature['PercentageShare']  # Replace 'PercentageShare' with your field name
        ...
        for subbasin_id, total in sums_by_subbasin.items():
            print(f"SubBasinId {subbasin_id}: Total PercentageShare = {total}")
        #delete untill here
    ...
    def selectOutputFolder(self):
        options = QFileDialog.Options()
        options |= QFileDialog.ReadOnly #includes
        self.outputFolder = None
    ...
    def run(self):
        ...
        #Log all QGIS INFO messages to QTalsim-Tab
        #self.setup_logging()
        ...
        #self.dlg.checkboxIntersectShareofArea.stateChanged.connect( lambda: self.toggleComboBoxVisibility(self.dlg.comboboxIntersectEZGField,self.dlg.checkboxIntersectShareofArea))
        ...
        self.dlg.onSoilType.clicked.connect(self.showSoilTypes)
        ...
        self.dlg.onLanduseParameter.clicked.connect(self.landuseParameters)
        ...
        #self.dlg.landuseParameter.currentIndexChanged.connect(self.update_table)
        ...
        #Loader
        self.loadingLabel = QLabel(self.dlg.topWidget)
        self.loadingLabel.setAttribute(Qt.WA_TranslucentBackground)
        self.loadingLabel.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint)
        current_path = os.path.dirname(os.path.abspath(__file__))
        gif_path = os.path.join(current_path, "gifs", "loadingGif.gif")
        self.loadingGif = QMovie(gif_path)
        self.loadingLabel.setMovie(self.loadingGif)
        size = self.loadingGif.currentPixmap().size()
        ...
        if result:
            filename = 'TalsimInputFiles.gpkg'
            ...
            try:
                gpkg_path = os.path.join(self.outputFolder, filename)
                if os.path.exists(gpkg_path): #kontrollieren, wieso es nicht funktioniert
                    print(f"{gpkg_path} exists")
                    try:
                        os.remove(gpkg_path)
                    except Exception as e:
                ...
                self.log_to_qtalsim_tab(f"File {filename} was saved to this folder: {self.outputFolder}", Qgis.Info)
            except Exception as e:
